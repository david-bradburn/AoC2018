...
class world_map:
				...
				match value:
					...
					case _:
						...
						if (self.new_train_map[new_loc[0]][new_loc[1]] == None) and (self.train_map[new_loc[0]][new_loc[1]] == None):#no trains in spot
							...
							print(new_loc[1],new_loc[0])
							self.train_map[row_index][colum_index].explode()
							self.new_train_map[new_loc[0]][new_loc[1]] = self.train_map[row_index][colum_index]
	...
	def main(self):
		self.process_input()
		# self.track_display()
		while not self.colision_detected:
			self.run_tick()
			# self.track_display()
			# if (self.tick_counter == 117):
			# 	self.track_display()
			print("Tick {} {}".format(self.tick_counter, self.train_sum))
			self.train_sum = 0
			self.tick_counter += 1
